    ...
    d1: {
      ...
      nodes: [
        { id: 'database',    label: 'Database',          drillable: false },
        {
          ...
          tooltip: 'Click body to cross-link. Click ↓ to drill into the row-level model.',
          d2: {
            ...
            nodes: [
              ...
              {
                id: 'clustered_idx', label: 'Clustered index (PK)', drillable: true,
                tooltip: 'PK IS the physical row order. UUID PKs cause random 16KB page writes — severe fragmentation.',
                d3: {
                  label: 'C++ internals — clustered index',
                  nodes: [
              ...
              {
                id: 'secondary_idx', label: 'Secondary index', drillable: true,
                tooltip: 'Stores PK value at leaves. Secondary lookup = 2 B-tree traversals.',
                d3: {
                  label: 'C++ internals — secondary index',
                  nodes: [
                    { id: 'dict_index_t', label: 'dict_index_t', drillable: false, tooltip: 'Index metadata: name, space_id, root page no, n_fields, type (clustered/secondary). Lives in data dictionary.' },
                    { id: 'buf_page_t',   label: 'buf_page_t',   drillable: false, tooltip: '16KB InnoDB page descriptor. page_id (space_id + page_no), oldest_modification LSN, state, zip_size.' },
                  ],
              ...
              {
                id: 'mvcc_view', label: 'MVCC read view', drillable: true,
                tooltip: 'InnoDB MVCC via undo log — old versions stored in system tablespace, not in-place like Postgres.',
                d3: {
                  label: 'C++ internals — MVCC',
                  nodes: [
                    { id: 'trx_t',     label: 'trx_t',     drillable: false, tooltip: 'Transaction struct. trx_id, state (ACTIVE/COMMITTED/ABORTED), undo_no, lock_list, read_view pointer.' },
